...
from bemppUQ.foa.operators import function_product, surface_divergence, surface_gradient, trace_transformation
from bemppUQ.functions import tangential_trace, neumann_trace
import numpy as np

def multitrace_operator(grid, wave_number, 
        parameters=None, target=None, spaces='maxwell_primal'):
    ...
    if spaces == 'maxwell_primal':
        ...
        efie = electric_field(
            rwg_space, rwg_space,
            snc_space, wave_number, parameters=parameters)
        mfie = magnetic_field(
            rwg_space, rwg_space,
            snc_space, wave_number, parameters=parameters)
        ...
        blocked_operator =  bempp.api.operators.boundary.maxwell.multitrace_operator(grid, wave_number, parameters=parameters, target=target)
...
def multitrace_identity(grid, parameters=None, spaces='maxwell_primal'):
    ...
    if spaces == 'maxwell_primal':
        ...
        blocked_operator = bempp.api.operators.boundary.sparse.multitrace_identity(grid, parameters = parameters, spaces=spaces)      
    return blocked_operator
...
def multitrace_osrc(grid, wave_number, 
        parameters=None, target=None, MtEType="1", Np=1):
...
def assemble_operators(grid, k_int, k_ext, far_field_points, spaces='maxwell', osrc=False, Np=1, MtEType='1'):
    ...
    if osrc and spaces == 'maxwell_primal':
        ...
        transmission_operators = multitrace_int, multitrace_ext, osrc_int, osrc_ext, identity, electric_far, magnetic_far
    else:
        transmission_operators = multitrace_int, multitrace_ext, identity, electric_far, magnetic_far
   
    return transmission_operators


def evaluate_far_field(transmission_operators, eps_rel, mu_rel, k_ext, polarization, direction):
...
def evaluate_far_field_sd(base_grid, transmission_operators, eps_rel, mu_rel, k_int, k_ext, polarization, direction, solution, grid_fun):
    ...
    electric_incident = bempp.api.GridFunction(electric_space, fun=tangential_trace(k_ext, polarization, direction),
                                              dual_space=electric_dual)
    magnetic_incident = bempp.api.GridFunction(magnetic_space, fun=neumann_trace(k_ext, polarization, direction),
    ...
    far_field_p = 4. * np.pi * (-electric_far * sol_p[1] - magnetic_far * sol_p[0])
    return far_field_p